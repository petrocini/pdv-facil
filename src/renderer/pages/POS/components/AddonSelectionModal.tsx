      ...
      <div className="relative bg-white w-full max-w-2xl max-h-[90vh] rounded-3xl shadow-2xl flex flex-col overflow-hidden animate-in fade-in zoom-in-95 duration-200">
              ...
              <div key={link.id} className="bg-gray-50 rounded-2xl p-5 border border-gray-100">
                <div className="flex justify-between items-start mb-4">
                  ...
                  <div className="flex items-center gap-2">
                    <span className={`text-sm font-bold px-3 py-1 rounded-full transition-colors ${isMaxReached
                  ...
                  {group.addons.map((addon: any) => {
                    ...
                    return (
                      <div
                        ...
                        className={`p-4 rounded-xl border-2 transition-all flex justify-between items-center ${isSelected
                      ...
                      >
                        ...
                        <div className="flex items-center gap-1 ml-3">
                          <button
                            ...
                            onClick={(e) => {
                              e.stopPropagation();
                              handleDecrement(group.id, addon.id);
                            }}
                            className={`w-8 h-8 flex items-center justify-center rounded-lg transition-all ${addonQty === 0
                          ...
                          <span className={`w-8 text-center text-sm font-bold ${addonQty > 0 ? 'text-blue-700' : 'text-gray-400'
                            }`}>
                            {addonQty}
                          </span>
                          <button
                            ...
                            onClick={(e) => {
                              e.stopPropagation();
                              handleIncrement(group.id, addon.id, max);
                            }}
                            className={`w-8 h-8 flex items-center justify-center rounded-lg transition-all ${!canIncrement
                                ? 'text-gray-300 cursor-not-allowed'
                                : 'bg-white text-blue-600 shadow-sm hover:bg-blue-50 hover:shadow active:scale-95'
                              }`}
                          >
        ...
        <div className="p-6 bg-white border-t border-gray-100 shadow-[0_-10px_30px_-15px_rgba(0,0,0,0.1)] flex items-center gap-6">
          ...
          <button
            ...
            className={`flex-1 py-4 rounded-xl font-bold text-lg flex justify-between items-center px-6 transition-all shadow-lg ${isValid
