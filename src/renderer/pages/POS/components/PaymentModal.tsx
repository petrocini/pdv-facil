...
export default function PaymentModal({ isOpen, onClose, cartTotal, onConfirm, isProcessing }: PaymentModalProps) {
  ...
  const [step, setStep] = useState<'payment' | 'change'>('payment');
  ...
  const handlePrimaryAction = () => {
    if (step === 'payment') {
      ...
      if (amountReceived === '' || received < cartTotal) {
  ...
  const isButtonDisabled = isProcessing || (step === 'payment' 
    ? (!selectedMethod || ticketNumber === '') 
      ...
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in duration-200">
        <div className="flex justify-between items-center p-5 border-b border-gray-100">
          <h2 className="text-xl font-bold text-gray-900">
            {step === 'payment' ? 'Finalizar Pedido' : 'Calcular Troco'}
          </h2>
          <button 
            onClick={onClose}
        ...
        <div className="p-6 space-y-6">
          {step === 'payment' ? (
            ...
            <>
              <div className="text-center p-4 bg-gray-50 rounded-xl border border-gray-100">
                <p className="text-sm font-semibold text-gray-500 mb-1">Total da Venda</p>
              ...
              <div>
                ...
                <input 
                  ...
                  disabled={isProcessing}
                  autoFocus
                  className="w-full text-center text-3xl font-bold p-4 border-2 border-gray-200 rounded-xl focus:border-green-500 focus:ring-0 transition-colors"
                  placeholder="0.00"
                />
              </div>

              {amountReceived !== '' && Number(amountReceived) >= cartTotal && (
                <div className="text-center p-4 bg-green-50 rounded-xl border border-green-200 animate-in zoom-in">
                  <p className="text-sm font-semibold text-green-700 mb-1">Troco a devolver</p>
                  <p className="text-4xl font-black text-green-600">
                    R$ {(Number(amountReceived) - cartTotal).toFixed(2).replace('.', ',')}
                  </p>
                </div>
              )}
            </>
        ...
        <div className="p-5 border-t border-gray-100 bg-gray-50 flex gap-3">
          {step === 'change' && (
            <button
              ...
              disabled={isProcessing}
              className="py-4 px-6 rounded-xl font-bold text-lg text-gray-600 bg-gray-200 hover:bg-gray-300 transition-colors"
            >
              Voltar
            </button>
          )}
          ...
          <button
            ...
            className={`flex-1 py-4 px-6 rounded-xl font-bold text-lg flex justify-center items-center gap-2 transition-all duration-300 ${
              isButtonDisabled
                ? 'bg-gray-200 text-gray-400 cursor-not-allowed'
                : 'bg-green-500 hover:bg-green-600 text-white shadow-lg hover:-translate-y-1 active:scale-[0.98]'
            }`}
          >
            {isProcessing ? 'Processando...' : (step === 'payment' && selectedMethod === 'Dinheiro' ? 'Avançar para Troco' : 'Confirmar e Emitir')}
